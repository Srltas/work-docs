...
즉 앱이 게이트웨이 브로커에 직접 접속한 연결(DBLink 경유가 아님)에서는 prepare / execute / fetch / commit(END_TRAN) / close 수준만 동작한다. commit/rollback 자체는 되지만 SAVEPOINT와 XA_* 함수코드가 이 계층에 없다.
...
- 단, **앱이 게이트웨이 브로커에 직접 접속한 연결**(DBLink 경유가 아니라 앱의 JDBC URL이 게이트웨이 브로커를 직접 가리키는 경우)에서는 위 함수코드 제약 때문에 Savepoint, 배치, 생성키 조회 등이 동작하지 않는다. JDBC 4.2 작업 시 이 기능들은 이 연결에서 깔끔한 "미지원" 예외로 우아하게 degrade 되도록 설계해야 한다.
...
### 7. DBLink 트랜잭션: 기본은 비원자(원격 auto-commit), `dblink_auto_commit`로 제어
매뉴얼의 두 서술이 이 파라미터로 화해된다.

| `dblink_auto_commit` | 원격 DML 동작 | 로컬 트랜잭션과의 관계 |
|---|---|---|
| `yes` (기본값) | 각 dblink 질의가 실행 즉시 자동 커밋 | 원자적이지 않음. 로컬 rollback으로 원격이 롤백되지 않음 |
| `no` | dblink 포함 전체 트랜잭션 커밋 시 원격 질의도 함께 커밋 | 함께 커밋/롤백 (코드상 XA 2PC로 조정) |

- 매뉴얼 DBLink 페이지는 공통 제약으로 기본 동작만 명시한다: "로컬DB와 원격DB의 트랜잭션(commit, rollback)은 하나의 트랜잭션으로 처리되지 않는다. 원격DB의 DML 질의는 로컬 DB의 트랜잭션과 별개로 auto commit으로 동작한다." 이 페이지는 `dblink_auto_commit`을 언급하지 않는다.
- 설정 페이지의 `dblink_auto_commit` 기본값은 `yes`이고, `no`로 두면 "dblink를 포함한 전체 트랜잭션이 커밋될 때 원격 질의도 함께 커밋"된다.
- 코드 근거: `no` 경로에서 원격 연결을 트랜잭션 내내 재사용하고(`dblink_scan.c:601`), 커밋 시 XA 2PC로 조정한다(`dblink_2pc.c:103` `cci_xa_prepare`, `:143` `cci_xa_end_tran`, `log_2pc.c:79` 등록, `transaction_sr.c:84` "dblink transaction commit first", 실패 시 전체 abort). SAVEPOINT는 지원되지 않는다.
- 문서 간 불일치 주의: DBLink 페이지는 "하나의 트랜잭션으로 처리되지 않는다"고 무조건 기술하지만, 설정 페이지는 `no`면 함께 커밋된다고 한다. DBLink 페이지가 파라미터를 교차 참조하지 않아 기본값만 읽으면 오해하기 쉽다.
- 미확인: `no`의 원자적 커밋이 이종(게이트웨이) 대상에도 성립하는지는 불명확하다. 2PC가 XA(`cci_xa_prepare`)를 쓰는데 CGW 함수 테이블은 `XA_PREPARE`/`XA_END_TRAN`을 미지원하므로(`cas_cgw.c`), 이종 대상에서는 이 경로가 제약될 수 있다.

## 결론
CUBRID↔CUBRID DBLink는 게이트웨이 CAS를 경유하지 않고 일반 브로커(`cub_cas`)로 직결된다. 게이트웨이(`cub_cas_cgw`)는 이종 DBMS 접근 전용 ODBC 프록시이며, DBLink의 대상 종류 구분은 엔진 코드가 아니라 `CREATE SERVER`가 가리키는 브로커의 `APPL_SERVER` 타입이 결정한다. 엔진은 두 경우 모두 `__gateway=true`를 부가하지만, 이 플래그 자체가 경유를 강제하지는 않는다. JDBC 드라이버는 두 경우 모두 앱↔CUBRID 구간에만 관여하고 원격/게이트웨이 홉은 서버 내부에서 처리되어 투명하다. DBLink 트랜잭션은 기본값(`dblink_auto_commit=yes`)에서 원격이 문 단위 auto-commit이라 로컬과 원자적이지 않고, `no`로 두면 함께 커밋된다(이종 대상 원자성은 미확인).

## 다음 단계
- JDBC 4.2 작업 반영: 앱이 게이트웨이 브로커에 직접 접속한 연결에서 미동작하는 기능(Savepoint, 배치, 생성키 등)은 표준 미지원 예외로 degrade 되도록 설계에 명시
- (완료) 매뉴얼 확인: `dblink_auto_commit` 기본값 `yes`(원격 문 단위 auto-commit), `no`면 함께 커밋. DBLink 페이지는 이 파라미터를 언급하지 않아 기본 동작만 읽으면 트랜잭션 미지원으로 오해하기 쉬움
- (선택) `dblink_auto_commit=no`의 원자적 커밋이 이종(게이트웨이) 대상에도 성립하는지 실동작 확인 (CGW가 `XA_PREPARE` 미지원이라 제약 가능성)
...
- `src/query/dblink_2pc.c:103` (`cci_xa_prepare`), `:143` (`cci_xa_end_tran`), `src/transaction/log_2pc.c:79` (2PC 참여자 등록), `transaction_sr.c:84` (dblink commit first), `src/base/system_parameter.c:5077` (`dblink_auto_commit` 기본값 yes)
- 매뉴얼(ko 11.4): DBLink 트랜잭션 제약 https://www.cubrid.org/manual/ko/11.4/sql/dblink.html
- 매뉴얼(ko 11.4): `dblink_auto_commit` 파라미터 https://www.cubrid.org/manual/ko/11.4/admin/config.html
- 기준: CUBRID 11.5 소스, CUBRID 11.4 매뉴얼
